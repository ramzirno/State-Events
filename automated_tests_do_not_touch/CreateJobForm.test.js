...
describe("CreateJobForm Alert shown at correct times.", () => {
  it("should not have an Alert message on load", () => {
    ...
    const { container } = render(
      <CreateJobForm jobs={[]} setJobs={mockCallback} />
    );
...
describe("CreateJobForm form validation", () => {
  it("should not submit form if job title is less than 10 chars", () => {
    ...
    expect(alert).not.toHaveTextContent(
      /Title must be at least 10 characters/i
    );
  });
  ...
  it("should not submit form if qualifications is empty", () => {
    ...
    act(() => {
      fireEvent.change(qualificationsInput, {
        target: { value: "A Qualifications" },
      });
      fireEvent.click(submitButton);
    });
  ...
  it("should not submit form if date is in the future", () => {
    const mockCallback = jest.fn();
    const { container } = render(
      <CreateJobForm jobs={[]} setJobs={mockCallback} />
    );

    // Trigger submit without setting date
    const dateInput = screen.getByLabelText(/Date Posted/i);

    const submitButton = screen.getByText(/submit/i);
    act(() => {
      fireEvent.change(dateInput, {
        target: { value: dayjs("1990-01-01").format("MM/DD/YYYY") },
      });
      fireEvent.click(submitButton);
    });

    // Check for alert due to missing date
    const alert = container.querySelector(".MuiAlert-root");
    expect(alert).toHaveTextContent(/Date Posted must be in the future/i);

    // Set a valid date and submit again
    act(() => {
      fireEvent.change(dateInput, {
        target: { value: dayjs("3000-01-01").format("MM/DD/YYYY") },
      });
      fireEvent.click(submitButton);
    });

    // Ensure alert is no longer shown
    expect(alert).not.toHaveTextContent(/Date is required/i);
  });
